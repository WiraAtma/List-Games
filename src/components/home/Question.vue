...
import { ref } from 'vue';
    ...
    const handleClick = (id) => {
        questionClicked.value[id] = !questionClicked.value[id];
    }
...
<template>
    <div class="mb-10">
        ...
        <div
            ...
            :key="item.id"
        >
            <button @click="handleClick(item.id)" class="font-semibold">
                {{ item.question }}
            </button>

            <transition name="fade">
                <p v-if="questionClicked[item.id]" class="mt-2">{{ item.answer }}</p>
            </transition>
        </div>
    </div>
</template>
